...
mv "$SRC_HOME/scripts/allSynonyms.tsv" "$SRC_HOME/drug_database_original.tsv"
cat "$SRC_HOME/drug_database_original.tsv" | awk 'BEGIN {FS="\t"; OFS="\t"} NR==1 {print} NR!=1 {print tolower($1)"\t"$2}' > "$SRC_HOME/drug_database.tsv"
...
mv "$SRC_HOME/scripts/allSynonyms.tsv" "$SRC_HOME/food_database_original.tsv"
cat "$SRC_HOME/food_database_original.tsv" | awk 'BEGIN {FS="\t"; OFS="\t"} NR==1 {print} NR!=1 {print tolower($1)"\t"$2}' > "$SRC_HOME/food_database.tsv"

# remove pre files
rm -f  "$SRC_HOME/scripts/pre_hmdb_drug.tsv"  "$SRC_HOME/scripts/pre_hmdb_food_diterpenoids.tsv" "$SRC_HOME/drug_database_original.tsv" "$SRC_HOME/food_database_original.tsv"

# microbial compounds
cat "$SRC_HOME/scripts/microbial_pre_database.tsv" | awk 'NR==1 {print "Name\tinHouse_ID"} NR!=1 {print $0"\tcnic_"NR-1}' > "$SRC_HOME/scripts/microbial_pre_database_id.tsv"
python "$SRC_HOME/scripts/getAllSynonyms.py" "$SRC_HOME/scripts/microbial_pre_database_id.tsv"
cat <(echo -e "Name\tinHouse_ID") <(cat "$SRC_HOME/scripts/allSynonyms.tsv" | tail -n +2) |  awk 'NR==1{print} NR!=1{print tolower($0)}' > "$SRC_HOME/microbial_database.tsv"
rm "$SRC_HOME/scripts/allSynonyms.tsv" "$SRC_HOME/scripts/microbial_pre_database_id.tsv"
